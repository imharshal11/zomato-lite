    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center gap-2 w-full">
          <span className="text-sm font-medium text-[#6b6b6b] whitespace-nowrap shrink-0">Diet:</span>
          <div className="flex gap-1 bg-[#f1f0eb] rounded-xl p-1 flex-1 min-w-0">
            <button
              onClick={() => setVegFilter('all')}
              className={`flex-1 px-2 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-all duration-150 truncate ${
                vegFilter === 'all'
        ...
        <div className="flex items-center gap-2 w-full">
          <span className="text-sm font-medium text-[#6b6b6b] whitespace-nowrap shrink-0">Rating:</span>
          <select
            value={ratingFilter}
            onChange={(e) => setRatingFilter(e.target.value as 'all' | '1' | '2' | '3' | '4' | '5')}
            className="flex-1 min-w-0 pl-3 pr-8 py-1.5 bg-white border border-[#e5e7eb] rounded-xl text-sm text-[#1a1a1a] focus:outline-none focus:ring-2 focus:ring-[#e23744] focus:border-transparent appearance-none bg-no-repeat bg-right"
            style={{
